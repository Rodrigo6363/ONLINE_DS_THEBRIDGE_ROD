...
def evaluate_kmeans(X, k_range=(1, 10)):
    ...
    k_values = list(range(k_range[0], k_range[1] + 1))  # Asegurarse de que el rango de k sea entero
    ...
    # Anotar el "codo" en la curva (usando k con el valor de inercia mínima)
    elbow_k = k_values[inertias.index(min(inertias))]
    plt.annotate('Elbow/Codo',
                 xy=(elbow_k, min(inertias)),
                 xytext=(0.55, 0.55),
    ...
    silhouette_scores = []
    for model in kmeans_per_k[1:]:  # Empezamos desde k=2
        silhouette_scores.append(silhouette_score(X, model.labels_))
    
    # Rango para las puntuaciones de silueta
    silhouette_k_values = list(range(2, len(silhouette_scores) + 2))  # Ajustar rango de k para que coincida con silhouette_scores
    
    # Graficar la puntuación de silueta
    plt.figure(figsize=(8, 3))
    plt.plot(silhouette_k_values, silhouette_scores, "bo-")
    plt.xlabel("$k$", fontsize=14)
    plt.ylabel("Silhouette Score", fontsize=14)
    plt.show()
