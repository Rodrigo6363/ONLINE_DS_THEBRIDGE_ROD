...
def plot_pairplots_by_category(data, num_columns=None, cat_column=None, palette='Set1'):
    # Si num_columns es None, se seleccionan las columnas numéricas automáticamente
    if num_columns is None:
        num_columns = data.select_dtypes(include=['number']).columns.tolist()
    
    # Dividir las columnas numéricas en grupos de 5
    num_groups = math.ceil(len(num_columns) / 5)
    for i in range(num_groups):
        # Seleccionar el grupo actual de columnas
        cols = num_columns[i*5:(i+1)*5]
        
        # Crear el pairplot para el grupo actual
        if cat_column:
            sns.pairplot(data[cols + [cat_column]], hue=cat_column, palette=palette)
            plt.suptitle(f'Pairplot de características numéricas por {cat_column} (Grupo {i+1})', y=1.02)
        else:
            sns.pairplot(data[cols])
            plt.suptitle(f'Pairplot de características numéricas (Grupo {i+1})', y=1.02)
...
def analyze_correlation_and_covariance(data, corr_threshold=0.5, cov_threshold=1.0):
    """
    Analiza la relación entre variables numéricas utilizando correlación y covarianza.
    Filtra variables con base en umbrales dados y genera visualizaciones.

    Parámetros:
    - data: DataFrame de pandas con los datos.
    - corr_threshold: Umbral absoluto mínimo de correlación para considerar variables relacionadas.
    - cov_threshold: Umbral absoluto mínimo de covarianza para considerar variables relacionadas.

    Salida:
    - Muestra un heatmap de correlación y un pairplot de variables filtradas.
    """
    ...
    # Filtrar pares de variables con correlación y covarianza por encima de los umbrales
    high_corr_vars = set()
    
    for col in corr_matrix.columns:
        for row in corr_matrix.index:
            if col != row:  # Evitar la diagonal
                if abs(corr_matrix.loc[row, col]) >= corr_threshold and abs(cov_matrix.loc[row, col]) >= cov_threshold:
                    high_corr_vars.add(col)
                    high_corr_vars.add(row)

    # Filtrar el DataFrame con las variables seleccionadas
    filtered_data = num_data[list(high_corr_vars)]
    ...
    if len(high_corr_vars) > 1:
        ...
        plt.suptitle("Pairplot de Variables Filtradas", y=1.02)
        plt.show()
    else:
        print("No hay suficientes variables con correlación y covarianza significativas para graficar.")
...
def evaluar_modelos_regresion(X_train_scaled, y_train):
    ...
    usando el RMSE (error cuadrático medio raíz) como métrica.
    Se utiliza el scoring "neg_root_mean_squared_error", que devuelve el RMSE en negativo;
    para comparar, se transforma a su valor positivo.
    El mejor modelo es aquel con menor RMSE medio.
    """
    model_names = [
    ...
    for nombre, modelo in zip(model_names, model_set):
        print(f"Evaluando modelo: {nombre}...")
        try:
            # Se usa scoring "neg_root_mean_squared_error" y se convierte a RMSE positivo.
            scores = cross_val_score(modelo, X_train_scaled, y_train, cv=3, 
                                     scoring="neg_root_mean_squared_error", n_jobs=-1)
            rmse_medio = -np.mean(scores)
            metricas_cv[nombre] = scores
            valores.append(rmse_medio)
        except Exception as e:
            print(f"Error con el modelo {nombre}: {e}")
            metricas_cv[nombre] = None
            valores.append(np.inf)  # Si falla, asignamos infinito para que no se escoja este modelo.
    
    # Como se busca el menor RMSE, se selecciona el índice con el valor mínimo.
    ganador = model_names[np.argmin(valores)]
    print(f"\nEl modelo ganador es: {ganador} con un RMSE medio de {np.min(valores):.4f}\n")
    
    resultados_df = pd.DataFrame({
        "Modelo": model_names,
        "RMSE Medio": valores
    }).sort_values(by="RMSE Medio", ascending=True)
    print(resultados_df)
    
    plt.figure(figsize=(12, 6))
    sns.barplot(x="RMSE Medio", y="Modelo", data=resultados_df, palette="viridis")
    plt.title("Comparación de Modelos de Regresión - RMSE Medio", fontsize=16)
    plt.xlabel("RMSE Medio", fontsize=12)
    plt.ylabel("Modelos", fontsize=12)
    plt.grid(axis="x", linestyle="--", alpha=0.7)
    plt.show()
