...
def log_transform(df, columnas, add_value=1):
    """
    Aplica la transformación logarítmica natural a las columnas especificadas de un DataFrame.
    Suma un valor (por defecto 1) a cada elemento antes de aplicar el logaritmo para evitar
    problemas con ceros o valores negativos.

    Parámetros:
    - df: DataFrame de pandas.
    - columnas: Lista de nombres de columnas a transformar.
    - add_value: Valor que se suma a cada elemento antes de aplicar el logaritmo (por defecto, 1).

    Retorna:
    - Un nuevo DataFrame con las columnas transformadas.
    """
    df_transformed = df.copy()  # Copiamos el DataFrame original para no modificarlo directamente
    
    # Sumar el pseudoconteo para evitar log(0) o log de valores negativos
    df_transformed[columnas] = np.log(df_transformed[columnas] + add_value)
...
def plot_correlation_heatmap(df, figsize=(20,20), cmap='coolwarm'):
    """
    Genera un heatmap con la matriz de correlación de un DataFrame.

    Parámetros:
    - df: DataFrame de pandas.
    - figsize: Tamaño de la figura (por defecto (20,20)).
    - cmap: Esquema de colores del heatmap (por defecto 'coolwarm').

    Retorna:
    - Muestra el heatmap de correlación.
    """
